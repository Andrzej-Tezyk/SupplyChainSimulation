...
# Include the module file first
include("../src/SupplyChainSim.jl")
using .SupplyChainSim  # Note the dot before SupplyChainSim
...
europe_dc = Storage("Europe DC", "Germany", 
    ...
    Dict(smart_thermostat => 100, security_camera => 100, smart_lighting => 100))
...
# Add seasonal demand
function generate_seasonal_demand(horizon::Int64; base::Number=50.0, amplitude::Number=20.0)
    demand = Float64[]
    for t in 1:horizon
        seasonal_factor = Float64(amplitude) * sin(2π * t / 365)
        daily_demand = Float64(base) + seasonal_factor + rand(Poisson(10))
